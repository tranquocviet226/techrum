...
export class TokenFactoryService {
  ...
  async generateJwtToken(user: UserEntity): Promise<{ accessToken: string ,expiresIn: number }> {
    const payload = {
      userId: user.id
    }
  ...
  getTimeExpiresIn() : number {
    const timeToLife = parseInt(this.configService.get<string>('time_to_life')) | 30;
